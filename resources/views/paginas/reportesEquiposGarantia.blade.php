...
<body>
    ...
    @foreach($equiposGarantia as $key => $value)
        <tr>
            <th colspan="2">NOMBRE DEL EQUIPO</th>
            <td colspan="8" style="text-align:center;">{{$value->nombre_equipoGarantia}}</td>
        </tr>
        ...
        <tr>
            <td style="text-align:center; vertical-align : middle;">{{$value->cp_equipoGarantia}}</td>
            <td style="text-align:center; vertical-align : middle;">{{$value->marca_equipoGarantia}}</td>
            <td style="text-align:center; vertical-align : middle;">{{$value->modelo_equipoGarantia}}</td>
            <td style="text-align:center; vertical-align : middle;">{{$value->serie_equipoGarantia}}</td>
            <td style="text-align:center; vertical-align : middle;">{{ \Carbon\Carbon::parse($value->fecha_adquisicion_equipoGarantia)->format('d-m-Y')}}</td>
        ...
        <tr>
            ...
            <th style="text-align:center; vertical-align : middle;">DETALLE DEL MANTENIMIENTO REALIZADO</th>
            <th style="text-align:center; vertical-align : middle;width:15px;" colspan="5">PROXIMO MNTO (Obs)</th>
        </tr>
    @endforeach

    @foreach($cronogramaCalendarioEquiposGarantia as $key => $value_cronograma)
        <tr>
            ...
            <td style="text-align:center; vertical-align : middle;">Por Garantía</td>
            <td style="text-align:left; vertical-align : middle;">{{$value_cronograma->observacion}}</td>
