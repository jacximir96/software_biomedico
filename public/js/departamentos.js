...
var tablaDepartamentos = $("#tablaDepartamentos").DataTable({
    ...
    "drawCallback": function (settings) {
        var pageInfo = tablaDepartamentos.page.info();
        updatePagination(pageInfo);
        updateInfo(pageInfo);
    }
});

var buttons = new $.fn.DataTable.Buttons(tablaDepartamentos, {
    buttons: [
        {
            "extend": 'excel',
            "footer": false,
...
tablaDepartamentos.on('order.dt search.dt draw.dt', function () {
    tablaDepartamentos.column(0, { search: 'applied', order: 'applied' }).nodes().each(function (cell, i) { cell.innerHTML = tablaDepartamentos.page.info().start + i + 1;  })
}).draw();
...
$(document).on('click', '.datatable-pagination-list-item-link', function (e) {
    ...
    tablaDepartamentos.page(page).draw(false);
});

$('.datatable-input').on('keyup', function() {
    tablaDepartamentos.search(this.value).draw();
});

$('.datatable-selector').on('change', function() {
    var valor = $(this).val();
    tablaDepartamentos.page.len(valor).draw();
});
