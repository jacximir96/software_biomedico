var tablaEquiposGarantia = $("#tablaEquiposGarantia").DataTable({
    processing: false,
    serverSide: true,
    responsive: true,
    autoWidth: false,
    ajax: {
        url: ruta + "/obtenerequipogarantia",
    },
    columnDefs: [
        {
            searchable: true,
            ...
            render: function (data, type, full, meta) {
                return `
					<div class="btn-group text-center">
						...
						<button class="hover:text-danger eliminarRegistro" action="${ruta}/equiposGarantia/${data}" method="DELETE" pagina="equiposGarantia" aria-label="Eliminar">
							<svg class="fill-current" width="24" height="24" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    drawCallback: function (settings) {
        var pageInfo = tablaEquiposGarantia.page.info();
        updatePagination(pageInfo);
        updateInfo(pageInfo);
    },
    initComplete: function () {
        direccionEjecutivaFilter(tablaEquiposGarantia);
        departamentoFilter(tablaEquiposGarantia);
        estadoFilter(tablaEquiposGarantia);
        marcaFilter(tablaEquiposGarantia);
    },
});

var buttons = new $.fn.DataTable.Buttons(tablaEquiposGarantia, {
    buttons: [
        {
            extend: "excel",
            footer: false,
...
tablaEquiposGarantia
    .on("order.dt search.dt draw.dt", function () {
        tablaEquiposGarantia
            .column(0, { search: "applied", order: "applied" })
            .nodes()
            .each(function (cell, i) {
                cell.innerHTML = tablaEquiposGarantia.page.info().start + i + 1;
            });
    })
    .draw();
...
$(document).on("click", ".datatable-pagination-list-item-link", function (e) {
    ...
    tablaEquiposGarantia.page(page).draw(false);
});

$(".datatable-input").on("keyup", function () {
    tablaEquiposGarantia.search(this.value).draw();
});

$(".datatable-selector").on("change", function () {
    var valor = $(this).val();
    tablaEquiposGarantia.page.len(valor).draw();
});

function direccionEjecutivaFilter(tablaEquiposGarantia) {
...
function departamentoFilter(tablaEquiposGarantia) {
    tablaEquiposGarantia.columns(8).every(function () {
        var column = tablaEquiposGarantia.column(this, {
            search: "applied",
        });
        var select = $(
...
function estadoFilter(tablaEquiposGarantia) {
    tablaEquiposGarantia.columns(11).every(function () {
        var column = this;
...
function marcaFilter(tablaEquiposGarantia) {
    var select = $(
