...
var tablaAdministradores = $("#tablaAdministradores").DataTable({
	...
	columns:[	
		...
		{
            data: 'foto_administrador',
            name: 'foto_administrador',
            render: function (data, type, row) {
                var defaultImage = `${ruta}/img/administradores/administrador.png`;
                var imageUrl = data ? `${ruta}/${data}` : defaultImage;
                return `<img src="${imageUrl}" class="img-fluid rounded-circle" style="width:50px; height:50px;">`;
            }
        },
		{
			data:'nombreRol_administador',
			name:'nombreRol_administador',
			render: function (item) {
                return item.toUpperCase();
            }
		},
			...
			render: function(data, type, full, meta) {
				return `
					<div class="btn-group text-center">
						...
						<button class="hover:text-danger eliminarRegistro" action="${ruta}/administradores/${data}" method="DELETE" pagina="administradores" aria-label="Eliminar">
							<svg class="fill-current" width="24" height="24" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	"language": {
	  ...
	  "drawCallback": function (settings) {
		  var pageInfo = tablaAdministradores.page.info();
		  updatePagination(pageInfo);
		  updateInfo(pageInfo);
	}
});

var buttons = new $.fn.DataTable.Buttons(tablaAdministradores, {
    buttons: [
        {
            "extend": 'excel',
            "footer": false,
        ...
        },
        {
            ...
            customize: function(doc) {
                doc.content[1].table.widths = ['5%', '35%', '35%', '25%'];
...
tablaAdministradores.on('order.dt search.dt draw.dt', function () {
    tablaAdministradores.column(0, { search: 'applied', order: 'applied' }).nodes().each(function (cell, i) { cell.innerHTML = tablaAdministradores.page.info().start + i + 1;  })
}).draw();
$('#tablaAdministradores').on('click', '.editar-btn', function() {
    var id = $(this).data('id');

    $.get(ruta + '/administradores/json/' + id, function(data) {
		console.log(data);
        $('#nombre_administrador').val(data.name);
        $('#correo_administrador').val(data.email);
		$('#password_actual').val(data.password);
        $('#rol_administrador').val(data.id_rol);

		if (data.foto) {
            var imageUrl = `${ruta}/${data.foto}`;
			console.log(imageUrl);
            $('#preview_img_administrador').attr('src', imageUrl);
        } else {
            var defaultImage = `${ruta}/img/administradores/administrador.png`;
            $('#preview_img_administrador').attr('src', defaultImage);
        }

		$('#editForm').attr('action', ruta+`/administradores/${id}`);
    });
});
...
$(document).on('click', '.datatable-pagination-list-item-link', function (e) {
    ...
    tablaAdministradores.page(page).draw(false);
});

$('.datatable-input').on('keyup', function() {
    tablaAdministradores.search(this.value).draw();
});

$('.datatable-selector').on('change', function() {
    var valor = $(this).val();
    tablaAdministradores.page.len(valor).draw();
});
