var tablaProveedores = $("#tablaProveedores").DataTable({
	...
	columns:[	
		...
		{data:'razonSocial_proveedor' ,name:'razonSocial_proveedor'},
		{data:'estado_proveedor' ,name:'estado_proveedor'},
		{data:'direccion_proveedor' ,name:'direccion_proveedor'},
		{data:'distrito_proveedor' ,name:'distrito_proveedor'},
		{data:'provincia_proveedor' ,name:'provincia_proveedor'},
		{data:'departamento_proveedor' ,name:'departamento_proveedor'},
		{ 
			data: "id_proveedor",
			name: 'acciones',
			render: function(data, type, full, meta) {
				return `
					<div class="btn-group text-center">
						...
						<button class="hover:text-danger eliminarRegistro" action="${ruta}/proveedores/${data}" method="DELETE" pagina="proveedores" aria-label="Eliminar">
							<svg class="fill-current" width="24" height="24" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	"drawCallback": function (settings) {
		var pageInfo = tablaProveedores.page.info();
    ...
    buttons: [
        ...
        },
        {
            ...
            customize: function(doc) {
                doc.content[1].table.widths = ['5%', '10%', '25%', '5%', '25%', '10%', '10%', '10%'];
...
tablaProveedores.on('order.dt search.dt draw.dt', function () {
    tablaProveedores.column(0, { search: 'applied', order: 'applied' }).nodes().each(function (cell, i) { cell.innerHTML = tablaProveedores.page.info().start + i + 1;  })
}).draw();
$('#tablaProveedores').on('click', '.editar-btn', function() {
    var id = $(this).data('id');

    $.get(ruta +'/proveedores/json/' + id, function(data) {
        $('#id').val(data.id_proveedor);
...
$(document).on('click', '.datatable-pagination-list-item-link', function (e) {
    ...
    tablaProveedores.page(page).draw(false);
});
...
$('.datatable-selector').on('change', function() {
    var valor = $(this).val();
    tablaProveedores.page.len(valor).draw();
});
