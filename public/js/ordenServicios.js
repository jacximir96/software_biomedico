let tablaOrdenServicios = $("#tablaOrdenServicios").DataTable({
    processing: false,
    serverSide: true,
	responsive: {
        details: {
	...
	autoWidth: false,
	ajax : {
		url: ruta + "/obtenerequiposervicios",
	},
	...
	columns: [
		{
			...
			name: 'id_ordenServicio',
			render: function (data, type, row, meta) {
				return meta.row + 1;
			}
		},
		{
			data:'codigo_ordenServicio',
			name:'codigo_ordenServicio',
			render: function (item) {
                return item.toUpperCase();
            }
		}, 
		{
			data:'expediente_ordenServicio',
			name:'expediente_ordenServicio',
			render: function (item) {
                return item.toUpperCase();
            }
		},
		{
			data: function (row) {
				return moment(row.fecha_ordenServicio).format('DD/MM/YYYY');
			}
			,name:'fecha_ordenServicio'
		},
		{
			data:'monto_ordenServicio',
			name:'monto_ordenServicio',
			render: function (data, type, row) {
				return 'S/.' + parseFloat(data).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
			}
		},
		{
			data: 'pdf_ordenServicio',
            name: 'Acciones',
			render: function(data, type, full, meta) {
				if (full.pdf) {
                    ...
                    return'<button class="btn btn-default btn-sm" disabled>Sin archivo</button>';
                }	
			}
		},
		{ 
			...
			render: function(data, type, full, meta) {
				return `
					<div class="btn-group text-center">
						...
						<button class="hover:text-danger eliminarRegistro" action="${ruta}/ordenServicios/${data}" method="DELETE" pagina="ordenServicios" aria-label="Eliminar">
							<svg class="fill-current" width="24" height="24" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	"drawCallback": function (settings) {
		var pageInfo = tablaOrdenServicios.page.info();
		updatePagination(pageInfo);
		updateInfo(pageInfo);
	}
});

var buttons = new $.fn.DataTable.Buttons(tablaOrdenServicios, {
    buttons: [
        {
            "extend": 'excel',
            "footer": false,
            "title": 'Orden de servicios',
            "filename": 'ORDEN_SERVICIOS',
...
tablaOrdenServicios.on('order.dt search.dt draw.dt', function () {
    tablaOrdenServicios.column(0, { search: 'applied', order: 'applied' }).nodes().each(function (cell, i) { cell.innerHTML = tablaOrdenServicios.page.info().start + i + 1;  })
}).draw();
...
$(document).on('click', '.datatable-pagination-list-item-link', function (e) {
    ...
    tablaOrdenServicios.page(page).draw(false);
});

$('.datatable-input').on('keyup', function() {
    tablaOrdenServicios.search(this.value).draw();
});

$('.datatable-selector').on('change', function() {
    var valor = $(this).val();
    tablaOrdenServicios.page.len(valor).draw();
});
...
$('#tablaOrdenServicios').on('click', '.editar-btn', function() {
	...
	$.get( ruta + '/ordenservicio/json/' + id, function(data) {
		...
		const fileNameContainer = $('#file-name');
		if (data.pdf_ordenServicio) {
			const fileName = data.pdf_ordenServicio.split('/').pop();
			fileNameContainer.html('<strong>Archivos existentes:</strong> ' + fileName);
		} else {
			fileNameContainer.html('<strong>Archivos existentes:</strong> No hay archivo seleccionado');
		}
