        ...
        if(request()->ajax()){
            ...
            ->addColumn('acciones', function($data){
                ...
                '<div class="btn-group text-center">
                    <button data-modal-target="editarModal" class="open-modal-btn hover:text-primary editar-btn" data-id="' .$data->id_mantenimiento.'">
                        <svg
                        class="fill-current"
                        width="24"
                        height="24"
                    ...
                    <button class="hover:text-primary eliminarRegistro" action="'.url()->current().'/'.$data->id_mantenimiento.'" 
...
/*         echo "<pre>"; print_r($datos); echo "</pre>";
    ...
    $cantidadNotificacionesCronogramaNuevo = DB::select("SELECT COUNT(C.id_cronogramaGeneralNuevo) as cantidad FROM cronogramageneralnuevo C WHERE /*C.mes_cronogramaGeneralNuevo BETWEEN MONTH('2012-01-01') AND MONTH(NOW())
        ...
        public function showJson($id) {
            $tipoMantenimientos = TipoMantenimientosModel::find($id);
